...
void App_020_TIMER_FASTPWM_LED() {
...
void App_019_TIMER_CTC_ADC_POTENTIOMETER() {
  // SET CTC TIMER0
  ADC_Init();
  DIO_Set_Pin_Direction(PORTB, Pin3, Out);
  uint8 digital = 2;
...
/***********************************************************/
void App_017_TIMER_SQRWAVE_NORMAL() {
...
void App_016_ISR() { //TIMER0_OVF_vect
  ...
  //MUST BE WRITTEN IN MAIN.C FILE OUTSIDE OF MAIN FUNCTION
  //  #include <avr/interrupt.h>
  //  ISR(TIMER0_OVF_vect){ App_015_ISR()}
}
/***********************************************************/
void App_015_LED_BUTTON_INTERRUPT() {
	LED0_Init();
...
/***********************************************************/
void App_014_KEYPAD_LCD_INTERRUPT() {
  LCD_Init();
  LED1_Init();
  EXTI0_Set_Direction_Input();
  EXTI0_Set_Internal_Pull_Up();
...
void App_014_ISR() {
  ...
  LCD_Send_Char(x);
  //This must be included in the main.c outside main function
//   #include <avr/interrupt.h>
//   ISR(INT0_vect){ App_012_ISR_ADC_Timer()}
}
/***********************************************************/
void App_013_ADC_Timer_With_Interrupt() {
  ADC_Init();
  LCD_Init();
  Timer0_Init();
  //Timer0_Enable_Overflow_Interrupt();
...
void App_013_ISR_ADC_Timer() {
  ...
  //#include <avr/interrupt.h>
  //ISR(TIMER0_COMP_vect){return;}
  //ISR(ADC_vect){App_012_ISR_ADC_Timer();}
}
/***********************************************************/
void App_012_ADC_Timer_No_Interrupt() { 
	//ADC Triggered By Timer0 Polling Version
	LCD_Init();
	Timer0_Init();
...
/***********************************************************/
void App_011_ADC_Manual_Interrupt(){ // ADC with interrupt not polling
	ADC_Init();
	LCD_Init();
...
/***********************************************************/
void APP_010_AC_Automatic_work(){
	//ADC POT  RELAY
	ADC_Init();LCD_Init();Relay_Init();
	uint8 digital = 0;
	uint8 old_digital = 0;
	float analog = 0;
	sint8 string[10];
...
void App_008_ADC_LCD_Potentiometer() {
  ADC_Init();
  LCD_Init();
  Button_Init();
  uint8 digital = 0;
...
void App_004_SSEG_LEDS_Traffic_Light() {
  LED0_Init();
  LED1_Init();
  LED2_Init();
  SSEG_Init();
  uint8 count = 0, color = 1;
  while (1) {
    if (count == 0) {
      switch (color) {
      case 4:
