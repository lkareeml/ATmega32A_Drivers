...
/****************************   Setting The Interrupt  *******************************/
	...
	void UART_Send_String_Polling(uint16 *String){
		...
		while(String[count] != '\0'){
			UART_Send_Byte_Polling(String[count]);
			count++;
		}
	}
	void UART_Recieve_String(uint16 *String){
		uint8 count = 0;
		while(1){
			String[count] = UART_Receive_Byte();
	...
	void UART_Send_String_Polling(uint8 *String8){
		uint8 count8 = 0;
		while(String8[count] != '\0'){
			UART_Send_Byte_Polling(String8[count8]);
			count8++;
		}
	}
	void UART_Recieve_String(uint8 *String8){
		uint8 count8 = 0;
		while(1){
			String8[count8] = UART_Receive_Byte();
			if(String8[count8] == 4){// ASCII 4 is [END TRANSMISSION]
				break;
			}
			else{
				count8++;
